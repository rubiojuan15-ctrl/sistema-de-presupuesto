  ...
  async downloadFile(e) {
    try {
      ...
      try {
        ...
        if (e.progress && u.body) {
          const F = u.body.getReader(), y = parseInt(
            u.headers.get("content-length") || "0",
            10
      ...
      const h = URL.createObjectURL(l), c = document.createElement("a");
      return c.href = h, c.download = this.extractFilename(e.path, e.url), document.body.appendChild(c), c.click(), document.body.removeChild(c), URL.revokeObjectURL(h), {
        blob: l,
        path: e.path
      };
    } catch (t) {
...
`).forEach((d) => {
            ...
            a(new Error(`HTTP error: ${r.status}`));
        }, r.onerror = () => {
          a(new Error("Network error occurred"));
        }, r.open(e.method || "POST", n, !0), e.headers && Object.entries(e.headers).forEach(([o, d]) => {
        ...
        i.append(l, t, e.path), e.params && Object.entries(e.params).forEach(([o, d]) => {
          Array.isArray(d) ? d.forEach((h) => i.append(o, h)) : i.append(o, d);
        }), r.send(i);
      });
    } catch (t) {
  ...
  async tryWriteToFilesystem(e, t) {
    ...
    try {
      ...
      if (r.length > 1) {
        ...
        await s.stat({ path: n }).catch(async () => {
          await s.mkdir({
            path: n,
            recursive: !0
          });
      ...
      return console.error("Error writing file to Filesystem:", s), !1;
    }
  }
  /**
